...
try {
  fs.copySync(templatePath, protoPath, { clobber: false, dereference: true })
} catch (e) {
  sh.error(e).exit(1)
}
...
function packageCustomization () {
  ...
  try {
    packagejson = fs.readFileSync(packagePath, 'utf8').replace('"browserify": {}', replace)
